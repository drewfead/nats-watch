...
function matchesKeyValueFilter(
  msg: NatsMessage | JetStreamMessage,
  ...
  switch (filter.operator) {
    case "contains":
      return (
        msg.headers?.[filter.key]?.some((header) =>
          header.includes(filter.value)
        ) ?? false
      );
  }
}
function matchesSubscriptionFilterSubject(
  msg: NatsMessage | JetStreamMessage,
  filter: ValueFilter
): boolean {
  switch (filter.operator) {
...
function matchesSubscriptionFilterPayload(
  msg: NatsMessage | JetStreamMessage,
  filter: ValueFilter
): boolean {
  switch (filter.operator) {
  ...
  switch (filter.type) {
    case "group":
      return filter.filters.reduce(
        (acc, f) => acc + countFiltersFromNode(f),
        0
      );
    case "value":
      return 1;
