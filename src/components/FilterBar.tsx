...
export function FilterBar({ filter, onClick, isOpen, onChange }: FilterBarProps) {
  const removeFilter = (path: FilterPath) => {
  ...
  const getFilterDescription = (filter: Filter, path: FilterPath = [], depth: number = 0): FilterDescriptionItem[] => {
    switch (filter.type) {
      case 'value':
        ...
        <span className="text-sm">
          {hasFilters ? (
            <div className="flex items-center flex-wrap">
              {filterDescriptions.map(({ text, path, groupType, isGroupStart, isGroupEnd, isOperator }, index) => (
                <span key={index} className={`flex items-center ${isOperator && !isGroupStart && !isGroupEnd ? 'mx-2' : ''} ${!isOperator ? 'whitespace-nowrap' : ''}`}>
                  {isGroupStart && (
                    <span className="text-gray-500 dark:text-gray-400 font-mono">(</span>
                  )}
                  {text && (
                    <span
                      className="inline-flex items-center px-2 py-1 rounded-md bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 text-xs font-medium"
                    >
                      {text}
                      <button
                        type="button"
