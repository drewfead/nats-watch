...
import { BoltIcon, ArrowTopRightOnSquareIcon } from '@heroicons/react/24/solid';
...
export function MessageDetail({ message, onClose, embedded = false, children }: MessageDetailProps) {
  ...
  const renderHeaderRow = (key: string, values: string[]) => (
    <tr key={key} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/50 transition-colors duration-150">
  ...
  const renderHeaders = () => {
    if (!message.headers || Object.keys(message.headers).length === 0) return null;
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              ...
              {initialHeaders.map(([key, values]) => renderHeaderRow(key, values))}
              {showAllHeaders && hiddenHeaders.map(([key, values]) => renderHeaderRow(key, values))}
            </tbody>
          </table>
          {hiddenHeaders.length > 0 && (
            ...
            >
              {showAllHeaders ? 'Show fewer headers' : `Show ${hiddenHeaders.length} more header${hiddenHeaders.length === 1 ? '' : 's'}`}
              <ChevronDownIcon 
          ...
          <div className="border-b dark:border-gray-700 p-4">
              ...
              {isJetStreamMessage && (
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  ...
                  <div className="group flex items-center gap-1.5">
                    <span className="font-mono text-gray-700 dark:text-gray-300">#{(message as JetStreamMessage).seq}</span>
                    <CopyButton 
            ...
            <div className="mt-1">
              <div className="flex flex-col text-sm text-gray-500 dark:text-gray-400">
                ...
                <time dateTime={message.timestamp} className="text-xs font-mono opacity-75 mt-0.5">
            ...
            <div className="flex-1">
              <div className="p-4">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-medium text-gray-900 dark:text-white">Body</h3>
                  <CopyButton 
